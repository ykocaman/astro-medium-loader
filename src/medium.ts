...
async function fetchMediumPosts(username: string): Promise<MediumPost[]> {
  ...
  const parser = new Parser();
    ...
    let feed: Parser.Output<Record<string, unknown>> | undefined;
  try {
  ...
  if (!feed) {
    throw new Error(`Failed to parse Medium RSS feed for @${username}: parser returned undefined`);
  }
  ...
  return feed.items.map((item: Parser.Item & { 'content:encoded'?: string; 'content:encodedSnippet'?: string }) => {
    ...
    const canonical: string = `<hr><p>Read the original post on: <a href="${item.link}" target="_blank">${item.title}</a></p>`;
